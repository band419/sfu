...
更糟的是：对 FP32 而言，当 $p = x\cdot \frac{4}{\pi}$ 的量级约为 $2^{31}$ 时，乘积的 ULP 约为

$$\mathrm{ulp}(p) \approx 2^{31-23} = 2^8 = 256$$

这意味着 $p$ 的整数低 8 位（当然也包括决定八分区的 $k\bmod 8$ 的低 3 位）在乘法结果中会被量化“抹平”，从而可能把 $k=\mathrm{round}(p)$ 的 $\bmod 8$ 算错。

一个具体反例（$x$ 与常量均为 FP32，乘法也按 FP32 舍入）：

- $x$（FP32）$= 1980432768.0$
- $(4/\pi)$（FP32）$\approx 1.2732394933700562$
- 参考（用高精度计算乘积再取整）：
  - $p_{\mathrm{ref}} = x\cdot \frac{4}{\pi} \approx 2521565315.906918 \Rightarrow k_{\mathrm{ref}}=2521565316,\ k_{\mathrm{ref}}\bmod 8 = 4$
- 直接 FP32 乘法：
  - $p_{32} = \mathrm{float32}(x\cdot (4/\pi)_{32}) = 2521565184.0 \Rightarrow k_{32}=2521565184,\ k_{32}\bmod 8 = 0$

可见仅用“固定精度常量 + 普通浮点乘法”会在大 $x$ 下把八分区（octant）判错。
